...
TXT
  ...
  describe "self.from_file" do
    it "should enter values from rows in text file" do
      board = Sudokill::Board.from_file("data/1.sud")
      (0..8).each do |row_i|
        (0..8).each do |col_i|
    ...
    it "should replace nums with zeros based on percent fill" do
      board = Sudokill::Board.from_file("data/1.sud", 0.25) # fill only 25% of the board

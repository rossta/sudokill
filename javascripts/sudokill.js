Sudokill = (function() {
  ...
  var instanceMethods = {
    ...
    listen: function(event, callback) {
      return this.$sudokill.bind(event, callback);
    },
    ...
    // Example Sudokill.game.connect("ws://linserv1.cims.nyu.edu:25252")
    ...
    status: function(msg, state) {
      ...
      self.$sudokill.trigger("game_state", state);
      ...
      if (message.match(/UPDATE|CREATE|SCORE|QUEUE|STATUS|COMMAND/)) {
        ...
        switch (json.action) {
          ...
          case "STATUS":
            ...
            break;
          case "COMMAND":
            self.$sudokill.trigger("game_command", json.command);
            break;
          default:
  ...
  classMethods = {
    play : function(selector, opts) {
      this.game = new Sudokill(selector, opts);
      return this.game;
    },
  ...
  GameBoard = Base.extend({
    constructor: function(domId, container, opts) {
      ...
      self.$sudokill.append(self.$form);
      self.$val.hide();
      self.$form.addClass(domId).submit(function() {
        ...
        if (self.isValid(val)) {
          self.$sudokill.trigger("send_message", ["MOVE", [row, col, val].join(" ")].join("|"));
          self.$val.hide();
        }
        return false;
      });
  ...
  Messager = Base.extend({
    constructor: function(selector) {
      ...
      $("#sudokill").bind("game_state", function(e, state) {
        switch (state) {
          case "in_progress":
            self.showStopButton();
            break;

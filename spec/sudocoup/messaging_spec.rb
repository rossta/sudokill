...
describe "Messaging" do
  describe Sudocoup::MessagePipe do
    before(:each) do
      @pipe = "|"
      @player_1 = mock_player(:number => 1)
      @player_2 = mock_player(:number => 2)
      @game = mock(Sudocoup::Game, :players => [@player_1, @player_2], :board => mock(Sudocoup::Board, :to_msg => "board_msg"), :size => 2)
    end
    describe "start_message" do
      it "should return START | player number | board" do
        message = Sudocoup::MessagePipe.start(@player_1, @game).split(@pipe)
    ...
    describe "reject_message" do
      it "should return REJECT | reason ... " do
        reason = "It failed"
        Sudocoup::MessagePipe.reject(reason).should == "REJECT#{@pipe}It failed"
      end
    end
    describe "game_over_message" do
      it "should return GAME OVER | reason... " do
        reason = "You lost"
        Sudocoup::MessagePipe.game_over(reason).should == "GAME OVER#{@pipe}You lost"
      end
    end
    describe "add_message" do
      it "should send MOVE | previous move | board" do
        message = Sudocoup::MessagePipe.add_move(@game).split(@pipe)
